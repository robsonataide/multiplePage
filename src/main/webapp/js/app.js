var app = (function() {
    ...
    .config(function($stateProvider, $urlRouterProvider, NotificationProvider) {
        ...
        $stateProvider
          ...
          .state('home.multiplePages', {
							url: "/{folder:[^/]+}/{name:[^/]+}/{params:.*}",
							controller: 'MultiplePageController',
							templateUrl: function (urlattr) {
								var param = "", params, action = "list";
								if (urlattr.params) {
									urlattr.params = urlattr.params.split('/');
									action = urlattr.params[0]!='list'?'form':urlattr.params[0];
								}
								return 'views/' + urlattr.folder + '/' + urlattr.name + '/' + action + '.view.html';
							}
					})
    ...
    // General controller
    .controller('MultiplePageController',["$scope","$stateParams","$location","$http", "$rootScope",function($scope, $stateParams, $location, $http, $rootScope){
      
      for(var x in app.userEvents)
        $scope[x]= app.userEvents[x].bind($scope);
      
      $scope.id = "";
      if($stateParams.params.length > 1)
        $scope.id = $stateParams.params[1];
      ...
      $scope.goList = function(){
        var splitVerb = ($location.path().indexOf('/edit')>=0)?'/edit':'/new';
        var listUrl = $location.path().split(splitVerb)[0]+'/list';
        $location.path(listUrl);
      };
      
      $scope.goEdit = function(id){
        var editUrl = $location.path().split('/list')[0]+'/edit/'+id;
        $location.url(editUrl);
      };
      
      $scope.goNew = function(){
        var newUrl = $location.path().split('/list')[0]+'/new';
