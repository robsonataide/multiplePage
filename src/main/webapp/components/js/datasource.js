...
.factory('DatasetManager', ['$http','$q', '$timeout','$rootScope', 'Notification', function($http, $q, $timeout, $rootScope, Notification) {
  ...
  */
  var DataSet = function(name, scope) {
    ...
    */ 
    ...
    /**
     * Store data in buffer and reset the buffer in this context
     */
  ...
  /**
    ...
    */
    this.goTo = function (rowId) {
      for(var i = 0; i < this.data.length; i++) {
        if(this.data[i][this.keys] === rowId) {
          cursor = i;
          this.active = this.copy(this.data[cursor],{});
          return this.active;
        }
    ...
    */
    ...
    /** 
     *  reload data with filters specified
     */
    this.reload = function() {
        this.history.length = 0;
        if (this.filterURL && this.filterURL.length > 0) {
            this.filter(this.filterURL);
        } else {
            this.filter("");
        }
    }
      ...
      this.$promise = $http({
        ...
        var sucessHandler = function (data) {
          ...
          */
          ...
          if(this.initialState && this.initialState == 'form'){
            var queryParams = this.$scope.$parent.params;
            if(queryParams.length > 1){
              this.goTo(queryParams[1]);
              this.startEditing();
            }else{
              this.startInserting();
            }
  ...
  */
  this.initDataset = function (props, scope) {
      ...
      dts.initialState = props.initialState;
    ...
    link: function( scope, element, attrs ) {
      ...
      var originPath = "origin-path:" + $location.path();
        ...
        var props = {
          ...
          initialState : attrs.initialState
        }
